...
- `python prune_chats.py --prune` — execute the deletion after reviewing the dry-run output. For each candidate the cascade is: delete classifications → delete `chats` row → add to ignore list → delete vault note. For manifest-marked chats, the JSON file itself is also deleted (safely — see below) before this cascade runs.
