...
necessary. If you regularly have very long conversations and want the
whole thing classified rather than truncated, that needs chat-splitting
logic this version doesn't implement — a known limitation, not a bug.
...
- `python prune_chats.py --unignore <cid> [<cid> ...]` — remove conversation IDs from the ignore list. The chats will be re-downloaded on the next extraction run (but their old classification and vault note are **not** restored — those must be regenerated by re-running the full pipeline).
...
| I accidentally pruned a conversation and want it back | Run `prune_chats.py --unignore <cid>` to remove it from the ignore list. The next extraction will re-download it. Re-run classify and vault afterwards — the old classification and vault note are **not** restored. |
